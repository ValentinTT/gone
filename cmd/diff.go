...
func init() {
	...
	diffCmd.Flags().BoolP("today", "t", false, "dates format/layout")
}
...
func diffTwoDates(cmd *cobra.Command, args []string) {
	layout_flag, _ := cmd.Flags().GetString("layout")
	today_flag, _ := cmd.Flags().GetBool("today")

	if !isValidFlag(layout_flag) { // error
		return 
	}
...
func isValidFlag(layout_flag string) bool {
